...
export function checkOkStatus(response: AxiosResponse<any>): boolean {
	return MathUtils.clampInclude(response.status, 200, 226)
};
...
export function checkokstatus<TResponse extends AxiosResponse<any>>(
	response: TResponse,
	ok: ArrayOr<number> | ((response: TResponse) => boolean) = checkOkStatus
): TResponse {
	/**
	 * Check is function
	 */
